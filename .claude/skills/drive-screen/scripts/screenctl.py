...
if OS == "Windows":
    ...
    def type_text(text: str) -> None:
        evs = []
        for ch in text:
            evs += [_char_ev(ch, False), _char_ev(ch, True)]
        for i in range(0, len(evs), 200):   # chunked: long strings drop otherwise
            _send(evs[i:i + 200])
            time.sleep(0.01)
...
def resolve(title: str) -> Win:
    """Return the ONE window whose title contains `title`, or refuse.
    ...
    """
    if not title:
        die("NO_TITLE", "--title is required for this action")
...
def focus(title: str, settle: float = 0.45) -> Win:
    ...
    if same_window(win):
        ...
        return resolve(title)          # re-read geometry: it may have been restored
