...
def pending_tool_calls(recs: list[dict]) -> list[str]:
    """Tool calls with no matching tool_result: what the session is waiting on.

    This is the signal that tells "finished" apart from "blocked at a prompt".
    From outside, both look identical: a still screen and a transcript that stopped
    growing. Confusing them is how an approval keystroke ends up typed into a
...
def cmd_wait(a) -> int:
    ...
    while time.time() < deadline:
        ...
        if not pending:
            _print_final(recs, f"quiet {int(idle)}s, nothing unanswered")
            return 0

        print(f"BLOCKED: quiet for {int(idle)}s with "
              f"{len(pending)} unanswered tool call(s).")
        print(f"Waiting on: {', '.join(sorted(set(pending)))}")
        print(f"RECORDS: {len(recs)}")
        print("A permission prompt is almost certainly on screen. Screenshot it,")
        print("read which command it is asking about, and answer deliberately.")
        return 2
